...
void hash::chaining()
{
	...
	{
		t[address].ph_no=ph_no;
		//strcpy(t[address].name,name);
	}
	else
	{
		//t[address].ph_no%10!=address
...
void hash::chaining_with()
{
	...
	if(k%max==address)              //hashed location contains synonym
    {
			...
			j=address,a=address;
			while(t[j].ph_no!=-1&&count<max)
            {
                j=(j+1)%max;
                if((t[j].ph_no%max)==address)           //to check the number of synonyms present.
                {
                    a=j;
                }
                count++;
